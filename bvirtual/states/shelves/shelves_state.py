...
class ShelvesState(rx.State):
    ...
    def list_shelves(self):
        ...
        with rx.session() as session:
            ...
            query = select(Shelves)
            
            
            # Aplicar la lógica de búsqueda si existe un término
            if self.search:
                query = query.where(Shelves.description.contains(self.search))
                self.shelves = session.exec(query).all()[::-1]
            else:
                ...
                self.shelves = session.exec(query).all()  