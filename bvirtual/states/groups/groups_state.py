...
class GroupsState(rx.State):
    ...
    def _get_total_items(self):
        ...
        with rx.session() as session:
            self.total_items = session.exec(
                select(func.count(User_Role.id))
            ).one()
    ...
    def list_groups(self):
        ...
        with rx.session() as session:
            ...
            if self.search:
                query = query.where(User_Role.name.contains(self.search))
                self.groups = session.exec(query).all()[::-1]
            else:
                ...
                self.groups = session.exec(query).all()  